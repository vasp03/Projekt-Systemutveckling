...
public class DayTimeManager : GameManagerBase, ITickable {
    ...
    private readonly ICollection<IDayTimeCallback> Callbacks = [];
    ...
    public static DAY_STATE GetCurrentDayState(int ticks) {
        ...
        if (ticks >= DayDurationRatio(DayDuration) && ticks < DayDurationRatio(DayDuration) * 3) // Morning
            return DAY_STATE.Morning;

        if (ticks >= DayDurationRatio(DayDuration) * 3 && ticks < DayDurationRatio(DayDuration) * 7) // Day
            return DAY_STATE.Day;

        if (ticks >= DayDurationRatio(DayDuration) * 7 && ticks < DayDurationRatio(DayDuration) * 9) // Evening
            return DAY_STATE.Evening;

        if (ticks >= DayDurationRatio(DayDuration) * 9 && ticks <= DayDuration) // Night
            return DAY_STATE.Night;
